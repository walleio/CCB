...
# shuffle the columns so that we are using random properties
...
if experiment == 'baseline':
    classifier = catboost.CatBoostClassifier(iterations=100, depth=2, learning_rate=0.1, loss_function='Logloss', task_type='GPU')
    classifier.fit(outputs_train, y=train_df['Y'], eval_set=(outputs_val, val_df['Y']), verbose=False)
    predictions = classifier.predict(outputs_test)
    ...
    for i, col in tqdm.tqdm(enumerate(random.sample(list(train_df.drop(columns=['Drug', 'Y']).columns), num_concepts)), desc='Training concept regressors'):
        classifier = catboost.CatBoostRegressor(iterations=100, depth=2, learning_rate=0.1, loss_function='MAE', task_type='GPU')
    ...
    means = train_df.drop(columns=['Drug', 'Y']).mean().values
    stds = train_df.drop(columns=['Drug', 'Y']).std().values
